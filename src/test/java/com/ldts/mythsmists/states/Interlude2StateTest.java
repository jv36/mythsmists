...
public class Interlude2StateTest {
    private Interlude2State state;
    private Interlude2Controller mockController;
    private TextSectionViewer mockViewer;

    @BeforeEach
    public void setUp() {
        mockController = Mockito.mock(Interlude2Controller.class);
        mockViewer = Mockito.mock(TextSectionViewer.class);

        state = new Interlude2State(new TextSection()) {
            @Override
